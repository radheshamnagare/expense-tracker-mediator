...
public class ManageExpenseTracker {
    ...
    private MeditaorExpenseTrackerDetailsResponse getExpenseTrackerDetailsResponse(List<ExpenseTrackerBean> expenseTrackerBeanList, List<FailRespose> fail, SystemError error, String sessionId, String token) {
        MeditaorExpenseTrackerDetailsResponse meditaorExpenseTrackerDetailsResponse = new MeditaorExpenseTrackerDetailsResponse();
        try {
            LOGGER.printf(Level.INFO, "Entry in getExpenseTrackerDetailsResponse()");
            meditaorExpenseTrackerDetailsResponse.setExpenseTrackerBeanList(expenseTrackerBeanList);
            ResponseStatus status = CommonService.getResponseStatus(error, sessionId, token);
            meditaorExpenseTrackerDetailsResponse.setStatus(status);
            meditaorExpenseTrackerDetailsResponse.setFail(fail);
        ...
        return meditaorExpenseTrackerDetailsResponse;
    }

    public MeditaorExpenseTrackerDetailsResponse manageExpenseTrackerDetails(ExpenseTrackerListReq expenseTrackerListReq) {
        ...
        try {
            ...
            if (!CommonService.isValidSessionToken(apiKey, token)) {
                ...
                ExpenseTrackerDetailsResponse expenseTrackerDetails =
    ...
    private boolean isValidId(String id, List<FailRespose> fail) {
        try {
            ...
            if (CommonValidator.isEmpty(id) || id.equals("0")) {
